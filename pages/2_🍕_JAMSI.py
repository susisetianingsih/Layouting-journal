...
def fill_template(template_file, journal_info, loa_id, current_date, loa_name, romawi_bulan, tahun, vol_no):
    ...
    output_path_word = os.path.join("D:\\PROJECT\\JAMSI\\LoA", f"{loa_name}.docx")
    output_path_pdf = os.path.join("D:\\PROJECT\\JAMSI\\LoA", f"{loa_name}.pdf")
...
def extract_loa_name(file_name):
    ...
    if match:
        ...
        return loa_id, f"LOA JAMSI_{loa_id} {loa_name}"
...
st.title("Ekstraksi Data JAMSI")

col1, col2 = st.columns(2)

with col1:
    romawi_bulan = st.text_input("Romawi bulan:", "IX")

with col2:
    tahun = st.text_input("Tahun:", "2024")
    
vol_no = st.text_input("Volume Nomor:", "Volume 4 Nomor 6 November 2024")
...
if uploaded_file is not None:
    ...
    TEMPLATE_PATH = "templates/LOA JAMSI_.docx"
