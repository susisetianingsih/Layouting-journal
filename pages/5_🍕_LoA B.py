...
st.set_page_config(
    page_title="LoA like JAMSI",
    page_icon="🍕"
)

# Extract title, author, and afiliation from docs
def extract_info_from_docx(file):
    ...
    if len(paragraphs) > 1:
        journal_info["author"] = paragraphs[1].text
    if len(paragraphs) > 3:
        journal_info["afiliasi"] = paragraphs[3].text
...
# Format author like [name author, afiliasi]
def format_authors(authors, affiliation):
    # Extract university name from the affiliation string
    university_name = re.search(r'\b(Sekolah|Institut|Politeknik|Universitas|University|Poltekkes|STIKES|Akademi|IAIN|STIE)\b.*?, Indonesia', affiliation).group(0)[:-11]
...
# Extract LoA name from file upload
...
# Streamlit UI
st.title("Ekstraksi Data LoA type JAMSI")
...
with col1:
    journal = st.selectbox(
        "Jurnal:",
        ("JAMSI", "IJPM", "JIPPM", "JPMII"),
    )
    
with col2:
    jamsi = ("I", "III", "V", "VII", "IX", "XI")
    ijpm = ("IV", "VIII", "XII")
    jippm = ("VI", "XII")
    jpmii = ("II", "IV", "VI", "VIII", "X", "XII")
    ...
    if journal == "JAMSI":
        ...
        bulan_index = jamsi.index(romawi_bulan)
        ...
        bulan_index = ijpm.index(romawi_bulan)
        tahun_dict = {2024: 2, 2025: 3, 2026: 4, 2027: 5, 2028: 6, 2029: 7, 2030: 8}
    elif journal == "JIPPM":
        romawi_bulan = st.selectbox(
            "Romawi bulan:",
            jippm,
        )
        bulan_index = jippm.index(romawi_bulan)
        tahun_dict = {2024: 4, 2025: 5, 2026: 6, 2027:7, 2028: 8, 2029: 9, 2030: 10}
    elif journal == "JPMII":
        romawi_bulan = st.selectbox(
            "Romawi bulan:",
        ...
        bulan_index = jpmii.index(romawi_bulan)
        tahun_dict = {2024: 2, 2025: 3, 2026: 4, 2027: 5, 2028: 6, 2029: 7, 2030: 8}
...
if uploaded_file is not None:
    ...
    # Fill template with data extract
